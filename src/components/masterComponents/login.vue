<template>
  <div id="loginContainer" :style="bgStyle">
    <headerComponents active-index="2" title="Bioinformatic of Cotton" ></headerComponents>
      ...
      handleLogin(ruleform){
        ...
        (reponse)=>{
          ...
          if(reponse.body.authenticateThrought==="yes"){
            //认证成功，进入个人主页
            this.$router.push({name:"showPage",params:{name:this.input.name}})
          }
    ...
    computed: {
      bgStyle(){
        if(window.innerWidth>=420){
          ...
          "background-image": "url(http://cotton.hzau.edu.cn/web/public/img/20190921231830.svg)"
        }
      }else{
        return {
          "background-color": "rgb(41, 45, 62)",
          "background-image": "url(http://cotton.hzau.edu.cn/web/public/img/login-sm-background.jpg)"
        }
      }
    }
    },
